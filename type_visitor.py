...
class TypeVisitor(visitor.Visitor):
    ...
    def _(self, node: ast.VarDefNode):
        ...
        if not self.t_env.is_assign_comp(node.value.get_type_str(), node.var.id_type.__str__()):
            self.type_error(node, node.value.get_type_str(), node.var.id_type.__str__())
    ...
    def _(self, node: ast.ReturnStmtNode):
        if node.expr is not None:
            ...
            scope_st = self.t_env.get_scope_symbol_table()
            signature = self.get_signature(scope_st)
            if signature.return_type != node.expr.get_type_str():
                self.type_error(node, node.expr.get_type_str(), self.current_return_type[-1])


    @visit.register
    def _(self, node: ast.UnaryOpExprNode):
        self.do_visit(node.operand)
        if node.operator == '-' and node.operand.get_type_str() != 'int':
            self.type_error(node, node.operator.get_type_str(), 'minus operator for non integer operand')
        elif node.operator == 'not' and node.operand.get_type_str() != 'bool':
            self.type_error(node, node.operand.get_type_str(), 'not operator for non boolean operator')
        else:
    ...
    def _(self, node: ast.IfExprNode):
        self.do_visit(node.condition)
        if node.condition.get_type_str() != 'bool':
            self.type_error(node.condition, node.condition.get_type_str(), 'condition not a bool')
    ...
    def _(self, node: ast.IndexExprNode):
        ...
        type_ = ''
        if node.list_expr.get_type_str() != '[int]' and node.list_expr.get_type_str() != '[str]':
            self.type_error(node, node.list_expr.get_type_str(), 'expr not a list')
        elif node.list_expr.get_type_str() == '[int]':
            type_ = 'int'
        elif node.list_expr.get_type_str() == '[str]':
            type_ = 'str'

        self.do_visit(node.index)
        if node.index.get_type_str() != 'int':
            self.type_error(node, node.index.get_type_str(), 'index not a int')

        node.set_type_str(type_)
    ...
    def _(self, node: ast.ListExprNode):
        ...
        for e in node.elements:
            self.do_visit(e)
            type_ = e.get_type_str()
            list_.append(e.get_type_str())
            if i > 0 and list_[i] != list_[i-1]:
                self.type_error(node, node.elements.get_type_str(), 'elements of the list of a different type')
            i = i + 1
    ...
    def _(self, node: ast.AssignStmtNode):
        ...
        for t in node.targets:
            ...
            list_.append(t.get_type_str())

        for l in range(len(list_)-1):
            if list_[l] != list_[l+1]:
                self.type_error(node, node.targets[l+1].get_type_str(), 'targets not the same type')

        self.do_visit(node.expr)
        if node.expr.get_type_str() != list_[0] and node.expr.get_type_str() != '':
            self.type_error(list_[0], node.expr.get_type_str(), 'expression not the same type as target')

    @visit.register
    def _(self, node: ast.WhileStmtNode):
        self.do_visit(node.condition)
        if node.condition != 'bool':
            self.type_error(node, node.condition.get_type_str(), 'condition not the of bool type')
        for s in node.body:
            self.do_visit(s)
    ...
    def _(self, node: ast.ForStmtNode):
        ...
        if not self.t_env.is_list_type(node.iterable.get_type_str()):
            self.type_error(node, node.iterable.get_type_str(), 'iterable is not iterable')
        if node.iterable.get_type_str() == 'str':
            if not node.identifier.get_type_str() == 'str':
                self.type_error(node, node.iterable.get_type_str(), 'iterable is not iterable')
        for s in node.body:
            self.do_visit(s)
